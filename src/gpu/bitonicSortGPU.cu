    ...
    uint32_t *valuesGlobal, uint32_t arrayLength, int sortOrder)
{
    ...
    calcDataBlockLength(offset, dataBlockLength, arrayLength, THREADS_BITONIC_SORT, BITONIC_BLOCKS);
...
{
    unsigned int offset, dataBlockLength;
    calcDataBlockLength(offset, dataBlockLength, arrayLength, THREADS_GLOBAL_MERGE, MERGE_BLOCKS);
...
void bitonicSortParallel(uint32_t *d_values, unsigned int array_length, int sortOrder)
{
    ...
    unsigned int elemsPerBlockBitonicSort = array_length / (THREADS_BITONIC_SORT * BITONIC_BLOCKS);
